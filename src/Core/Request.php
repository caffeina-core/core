...
class Request {
  ...
  public static function host($with_protocol=true){
    ...
    return ($with_protocol
      ? 'http' . (!empty($_SERVER['HTTPS'])&&(strtolower($_SERVER['HTTPS'])!=='off')?'s':'') . '://'
      : ''
    ) . static::filterWith('host', $host);
  }
  ...
  public static function method(){
   return static::filterWith('method',strtolower(empty($_SERVER['REQUEST_METHOD'])?'get':$_SERVER['REQUEST_METHOD']));
  }
  ...
  public static function UA(){
   return static::filterWith('UA',strtolower(empty($_SERVER['HTTP_USER_AGENT'])?'':$_SERVER['HTTP_USER_AGENT']));
  }
