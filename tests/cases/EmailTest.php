...
class EmailTest extends PHPUnit_Framework_TestCase {
  ...
  public function testSetterGetters() {
    ...
    foreach (['to', 'cc', 'bcc'] as $key) {
      ...
      $mail->$key('silvia@test.com');
      $this->assertEquals(implode('|', $mail->$key()), 'stefano@test.com|silvia@test.com', "SetterGetter : $key");
      $mail->$key(false);
    ...
    foreach (['from', 'replyTo', 'subject', 'contentType', 'message'] as $key) {
      ...
      $mail->$key('silvia@test.com');
      $this->assertEquals($mail->$key(), 'silvia@test.com', "SetterGetter : $key");
      $mail->$key(false);
  ...
  public function testEmailFacade() {
    ...
    if (!$success) {
      $this->fail("Email.Send(array) failed.");
    }
    ...
    if (!$success) {
      $this->fail("Email.Send(envelope) failed.");
    }
